...
export async function getArticles() {
  ...
  const mainFeaturedNews = processedNews.find(n => n.featureStatus === 'destacada') || null;
  const secondaryFeaturedNews1 = processedNews.find(n => n.featureStatus === 'noticia2') || null;
  const secondaryFeaturedNews2 = processedNews.find(n => n.featureStatus === 'noticia3') || null;

  const otherNews = processedNews.filter(n => 
  ...
  return {
    ...
    otherNews,
    allNews: processedNews, // Also return all news for other uses
  };
}
